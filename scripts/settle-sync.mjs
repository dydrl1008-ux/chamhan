...
const today = iso(kst()); const TO = env.TO || today;
const fyStart = (d) => { const y = Number(d.slice(0, 4)); const s = `${y}-12-21`; return d >= s ? s : `${y - 1}-12-21`; };   // 회사 연도 = 12/21 시작
const FROM = env.FROM || fyStart(TO);
