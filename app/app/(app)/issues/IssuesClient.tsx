  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      ...
      {canWrite && <div className="card"><h3 style={{ margin: '0 0 12px', fontSize: 15 }}>이슈 등록</h3>
        <form action={async fd => { const x = await addIssue(fd); setMsg(x.msg); if (x.ok) { (document.getElementById('issueForm') as HTMLFormElement)?.reset(); r.refresh(); } }} id="issueForm" style={{ display: 'grid', gap: 10 }}>
