...
export default function ApproveModal({ settlementSeq, onClose }: { settlementSeq: string; onClose: () => void }) {
  ...
  useEffect(() => { loadApprove(settlementSeq).then(x => { if (!x.ok) setErr(x.msg); else { setSt({ d: x.d, row: x.row }); setAmt(String(x.d.confirmAmt)); setCalc(x.d); } }); }, [settlementSeq]);
  ...
  const submit = async () => { if (!st) return; if (!st.d.refund && !st.d.fixedAmt && amt.trim() === '') return notify('입금금액을 입력하세요', 'bad'); if (!confirm(`${settlementSeq} 승인 처리하시겠습니까?\n정산 사이트에 즉시 반영됩니다.`)) return; setBusy(true); const x = await doApprove(settlementSeq, st.d.refund || st.d.fixedAmt ? null : Number(amt) || 0, remark); setBusy(false); notify(x.msg, x.ok ? 'ok' : 'bad'); if (x.ok) { onClose(); r.refresh(); } };
