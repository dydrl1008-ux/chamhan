...
import { dismissPending } from './actions';
...
export default function PendingList({ open, dismissed }: { open: P[]; dismissed: P[] }) {
  const r = useRouter(); const [sel, setSel] = useState<Set<string>>(new Set()); const [showDis, setShowDis] = useState(false);
  ...
  const Row = ({ p, dis }: { p: P; dis?: boolean }) => <tr key={p.item_key} style={{ opacity: dis ? .6 : 1 }}>
    ...
    <td style={{ whiteSpace: 'nowrap' }}>{dis ? <button className="btn ghost" style={{ padding: '3px 8px', fontSize: 11 }} onClick={() => act([p.item_key], false)}>복원</button> : <button className="btn ghost" style={{ padding: '3px 8px', fontSize: 11 }} onClick={() => act([p.item_key], true)}>알림 해제</button>}</td></tr>;
  ...
  return <>
    <div className="card">
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}><span style={{ fontSize: 13, fontWeight: 600 }}>대기 {open.length}건</span>{sel.size > 0 && <button className="btn" style={{ padding: '5px 12px', fontSize: 12.5 }} onClick={() => { if (confirm(`${sel.size}건 알림 해제할까요? 목록·배지에서 빠지고, 정산 사이트에서 승인되면 처리됨으로 정리됩니다.`)) act([...sel], true); }}>선택 {sel.size}건 알림 해제</button>}<span style={{ fontSize: 12, color: 'var(--muted)' }}>이전에 안 할 건은 알림 해제로 넘기세요. 승인/반려는 정산 사이트에서.</span></div>
