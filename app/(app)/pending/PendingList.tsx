...
export default function PendingList({ open, dismissed }: { open: P[]; dismissed: P[] }) {
  ...
  const Row = ({ p, dis }: { p: P; dis?: boolean }) => <tr key={p.item_key} style={{ opacity: dis ? .6 : 1 }}>
    ...
    <td style={{ whiteSpace: 'nowrap' }}>{dis ? <button className="btn ghost" style={{ padding: '3px 8px', fontSize: 11 }} onClick={() => act([p.item_key], false)}>복원</button> : <><button className="btn" style={{ padding: '3px 10px', fontSize: 11 }} onClick={() => setAppr(p.settle_no ?? '')}>승인</button> <button className="btn ghost" style={{ padding: '3px 8px', fontSize: 11, color: 'var(--bad)', borderColor: 'var(--bad-soft)' }} onClick={async () => { if (!p.settle_no) return; if (!confirm(`${p.settle_no} 반려(승인취소) 하시겠습니까?\n정산 사이트 상태가 '승인취소'로 바뀌고 직원에게 다시 요청하라고 알려야 합니다.`)) return; const x = await doCancel(p.settle_no); notify(x.msg, x.ok ? 'ok' : 'bad'); r.refresh(); }}>반려</button> <button className="btn ghost" style={{ padding: '3px 8px', fontSize: 11 }} onClick={() => act([p.item_key], true)}>알림 해제</button></>}</td></tr>;
  ...
  return <>
    ...
    <div className="card">
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}><span style={{ fontSize: 13, fontWeight: 600 }}>대기 {open.length}건</span>{gubuns.length > 1 && <select value={gub} onChange={e => setGub(e.target.value)} style={{ width: 150, padding: '4px 8px', fontSize: 12.5 }}><option value="">전체 구분</option>{gubuns.map(g => <option key={g} value={g}>{g} ({open.filter(p => p.req_gubun === g).length})</option>)}</select>}{sel.size > 0 && <button className="btn" style={{ padding: '5px 12px', fontSize: 12.5 }} onClick={() => { if (confirm(`${sel.size}건 알림 해제할까요? 목록·배지에서 빠지고, 정산 사이트에서 승인되면 처리됨으로 정리됩니다.`)) act([...sel], true); }}>선택 {sel.size}건 알림 해제</button>}<span style={{ fontSize: 12, color: 'var(--muted)' }}>승인/반려 = 정산 사이트에 바로 반영 (반려 = 사이트의 승인취소). 나중에 볼 건은 알림 해제.</span></div>
