...
export default async function PendingPage() {
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      ...
      <div className="card"><h3 style={{ margin: '0 0 10px', fontSize: 15 }}>최근 처리됨 <span style={{ fontSize: 12, color: 'var(--muted)', fontWeight: 400 }}>30건</span></h3><table><thead><tr><th>처리 시각</th><th>담당자</th><th>고객</th><th>상품</th><th style={{ textAlign: 'right' }}>영업이익</th><th>결과</th></tr></thead><tbody>
        {(done ?? []).map(p => <tr key={p.item_key}><td style={{ whiteSpace: 'nowrap' }}>{fmtKST(p.resolved_at)}</td><td>{p.empl_name}</td><td>{p.cust_name}</td><td>{p.prod_name}</td><td style={{ textAlign: 'right' }}>{won(p.amount)}</td><td><span className="pill" style={/승인완료/.test(p.resolved_status ?? '') ? { background: 'var(--ok-soft)', color: 'var(--ok)' } : { background: 'var(--bg)', color: 'var(--muted)' }}>{p.resolved_status}</span></td></tr>)}
        {(done ?? []).length === 0 && <tr><td colSpan={6} style={{ color: 'var(--muted)' }}>없음</td></tr>}</tbody></table></div>
    </div>
  );
}
