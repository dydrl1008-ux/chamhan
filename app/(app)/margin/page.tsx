...
export default async function MarginPage({ searchParams }: { searchParams: { t?: string; team?: string; m?: string } }) {
  ...
  const Tab = ({ k, l }: { k: string; l: string }) => <a href={`/margin?t=${k}&m=${key}`} style={{ padding: '7px 14px', borderRadius: 8, fontWeight: 600, fontSize: 13, textDecoration: 'none', background: tab === k ? 'var(--accent-soft)' : 'transparent', color: tab === k ? 'var(--accent)' : 'var(--muted)' }}>{l}</a>;
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}><h1 style={{ fontSize: 20, margin: 0 }}>영업 마진</h1><div style={{ display: 'flex', gap: 4, background: 'var(--panel)', border: '1px solid var(--line)', padding: 4, borderRadius: 12 }}><Tab k="day" l="일" /><Tab k="week" l="주" /><Tab k="month" l="월" /></div><div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><a href={`/margin?t=month&m=${prevBizMonth(key)}`} className="btn ghost" style={{ padding: '4px 10px', textDecoration: 'none' }}>‹</a><b>{bizLabel(key)}</b><a href={`/margin?t=month&m=${nextBizMonth(key)}`} className="btn ghost" style={{ padding: '4px 10px', textDecoration: 'none' }}>›</a></div><span style={{ fontSize: 12, color: 'var(--muted)' }}>월 = 21일~20일 마감 기준 · 금일 마진 = 정산 자동(÷1.1) + 직원 추가분 · VAT 제외</span></div>
      {agg}
