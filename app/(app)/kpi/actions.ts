...
export async function submitKpi(fd: FormData): Promise<{ ok: boolean; msg: string }> {
  ...
  const row = { user_id: me.id, work_date, calls: n('calls'), new_cnt: n('new_cnt'), margin_manual: n('margin_manual'), kakao_db: n('kakao_db'), overtime: fd.get('overtime') === '1', new_margin: n('new_margin'), work_report: String(fd.get('work_report') || ''), feedback: String(fd.get('feedback') || '') };
