...
export default async function WeeklyPage({ searchParams }: { searchParams: { team?: string; w?: string } }) {
  ...
  const we = addDays(ws, 6), ps = addDays(ws, -7), pe = addDays(ws, -1); const ms = ws.slice(0, 7) + '-01';
  const [{ data: members }, { data: kpi }, { data: report }, { data: pipeline }, { data: targets }] = await Promise.all([
    ...
    sb.from('kpi_daily').select('user_id,work_date,calls,new_cnt,margin,kakao_db,overtime,new_margin,work_report,feedback').eq('team_id', teamId).gte('work_date', ps < ms ? ps : ms).lte('work_date', we).order('work_date'),
    ...
    sb.from('monthly_targets').select('user_id,margin').eq('month', ms),
  ]);
  ...
  return <WeeklyClient me={me} teams={teams ?? []} teamId={teamId} ws={ws} we={we} ps={ps} pe={pe} weeks={weeks} members={members ?? []} kpi={kpi ?? []} report={report} notes={notes ?? []} pipeline={pipeline ?? []} targets={Object.fromEntries((targets ?? []).filter(t => t.user_id).map(t => [t.user_id!, Number(t.margin)]))} monthStart={ms} />;
}
