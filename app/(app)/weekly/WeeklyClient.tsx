...
export default function WeeklyClient(p: { me: Profile; teams: { id: number; name: string; leader_id: string | null }[]; teamId: number; ws: string; we: string; ps: string; pe: string; weeks: string[]; members: { id: string; name: string; position: string | null }[]; kpi: K[]; report: Rep; notes: { user_id: string; directive: string | null; feedback: string | null }[]; pipeline: P[]; targets: Record<string, number>; monthStart: string }) {
  const { me, teams, teamId, ws, we, ps, pe, weeks, members, kpi, report, notes, pipeline, targets, monthStart } = p;
  ...
  const cur = inW(ws, we), prev = inW(ps, pe), month = kpi.filter(x => x.work_date >= monthStart);
    ...
    <form action={async fd => { const x = await saveReport(fd); { notify(x.msg); setMsg(x.msg); }; if (x.ok) r.refresh(); }} style={{ display: 'grid', gap: 18 }}>
      ...
      <Sec b="A" t="팀 요약" d="이번주 / 전주 · 일간 KPI 자동 집계">
        <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 18 }}>
          ...
          <table><thead><tr><th>인원 (이달 누계)</th><th style={{ textAlign: 'right' }}>신규</th><th style={{ textAlign: 'right' }}>신규 마진</th><th style={{ textAlign: 'right' }}>마진</th></tr></thead><tbody>{members.map(m => { const rows = month.filter(x => x.user_id === m.id); return <tr key={m.id}><td><b>{m.name}</b></td><td style={{ textAlign: 'right' }}>{S(rows, 'new_cnt')}</td><td style={{ textAlign: 'right' }}>{won(S(rows, 'new_margin'))}</td><td style={{ textAlign: 'right', fontWeight: 700 }}>{won(S(rows, 'margin'))}</td></tr>; })}<tr style={{ fontWeight: 700, background: 'var(--bg)' }}><td>총합</td><td style={{ textAlign: 'right' }}>{S(month, 'new_cnt')}</td><td style={{ textAlign: 'right' }}>{won(S(month, 'new_margin'))}</td><td style={{ textAlign: 'right' }}>{won(S(month, 'margin'))}</td></tr></tbody></table>
        </div>
      </Sec>
      ...
      <Sec b="D" t="인원별 지시사항 · 피드백" d="공통 지시 + 인원별">
        ...
        {members.map(m => { const n = notes.find(x => x.user_id === m.id); const mo = S(month.filter(x => x.user_id === m.id), 'margin'); const tg = targets[m.id] ?? 0; return (
          <div key={m.id} style={{ display: 'grid', gridTemplateColumns: '170px 1fr 1fr', gap: 14, padding: '14px 0', borderBottom: '1px solid var(--line)', alignItems: 'start' }}>
            <div><input type="hidden" name="member_id" value={m.id} /><b>{m.name}</b><div style={{ fontSize: 11, color: 'var(--muted)' }}>{m.position}</div><div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 8 }}>이달 마진 / 목표</div><div style={{ fontWeight: 700 }}>{man(mo)} <span style={{ color: 'var(--muted)', fontWeight: 400 }}>/ {man(tg)}</span></div><div style={{ height: 6, background: 'var(--line)', borderRadius: 4, marginTop: 4 }}><div style={{ width: `${tg ? Math.min(100, mo / tg * 100) : 0}%`, height: '100%', background: mo / (tg || 1) >= 1 ? 'var(--ok)' : 'var(--accent)', borderRadius: 4 }} /></div></div>
