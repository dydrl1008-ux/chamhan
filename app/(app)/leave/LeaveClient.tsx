...
export default function LeaveClient({ me, people, leaves, monthly, yearly, teams, month, today, selUser }: { me: Profile; people: P[]; leaves: L[]; monthly: Agg[]; yearly: Agg[]; teams: { id: number; name: string }[]; month: string; today: string; selUser: string }) {
  ...
  const stLabel = (l: L) => l.status === 'pending' ? (['late','absent'].includes(l.type) ? ['총괄 승인 대기', { background: '#FDF1DD', color: '#D97706' }] as [string, React.CSSProperties] : l.team_approved_at ? ['팀승인 · 최종대기', { background: '#E0E7FF', color: '#3730A3' }] as [string, React.CSSProperties] : stName.pending) : stName[l.status];
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      ...
      {tgt && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
        {[[`${one ? tgt.name : '내'} 잔여 연차`, `${tgt.annual_leave_granted - ty.annual_used}일`, `부여 ${tgt.annual_leave_granted} · 사용 ${ty.annual_used} (연)`], [`${Number(month.slice(5))}월 지각`, `${tm.late_cnt}회`, `연 누계 ${ty.late_cnt}회`], [`${Number(month.slice(5))}월 연차·반차·병가`, `${tm.annual_cnt} · ${tm.half_cnt} · ${tm.sick_cnt}회`, `연 누계 ${ty.annual_cnt} · ${ty.half_cnt} · ${ty.sick_cnt}회`], [isFinal ? '최종 승인 대기' : canManage ? '팀 승인 대기' : '내 신청 대기', `${rows.filter(l => l.status === 'pending' && (isFinal ? true : canManage ? (!l.team_approved_at && !['late','absent'].includes(l.type)) : l.user_id === me.id)).length}건`, isFinal ? `팀승인 안 된 건 ${rows.filter(l => l.status === 'pending' && !l.team_approved_at).length}` : '']].map(([l, v, d]) => (
