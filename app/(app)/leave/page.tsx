...
export default async function LeavePage({ searchParams }: { searchParams: { m?: string; u?: string } }) {
  ...
  const [{ data: people }, { data: leaves }, { data: monthly }, { data: yearly }, { data: teams }] = await Promise.all([
    ...
    sb.from('leave_requests').select('id,user_id,type,start_date,end_date,days,reason,status,source,requested_by,decided_by,team_approved_at').gte('start_date', `${year}-01-01`).order('start_date', { ascending: false }),
