...
export default async function LeavePage({ searchParams }: { searchParams: { m?: string; u?: string } }) {
  ...
  const [{ data: people }, { data: leaves }, { data: monthly }, { data: yearly }, { data: teams }] = await Promise.all([
    ...
    sb.from('leave_requests').select('id,user_id,type,start_date,end_date,days,reason,status,source,requested_by,decided_by,team_approved_at').gte('start_date', `${Number(year) - 1}-12-21`).order('start_date', { ascending: false }),
    sb.from('v_leave_monthly').select('*').gte('month', `${year}-01-01`).lte('month', `${year}-12-01`),
    sb.from('v_leave_yearly').select('*').eq('year', Number(year)),
    sb.from('teams').select('id,name').eq('is_active', true).order('sort_order'),
  ]);
  return <LeaveClient me={me} people={(people ?? []).filter(p => p.role !== 'admin')} leaves={leaves ?? []} monthly={monthly ?? []} yearly={yearly ?? []} teams={teams ?? []} month={month} range={[bs, be]} today={today} selUser={searchParams.u ?? 'all'} />;
}
