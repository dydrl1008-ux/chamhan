      ...
      <div style={{ display: 'grid', gridTemplateColumns: isAdmin ? '1fr 1fr' : '1fr', gap: 18 }}>
        ...
        <div style={{ display: 'grid', gap: 18, alignContent: 'start' }}>
          <div className="card"><h3 style={{ margin: '0 0 10px', fontSize: 15 }}>3. 동기화 <span style={{ fontSize: 12, color: 'var(--muted)', fontWeight: 400 }}>매일 06:00 자동(최근 7일) + 수동</span></h3>
            ...
            <table style={{ marginTop: 12, fontSize: 12.5 }}><thead><tr><th>시각</th><th>기간</th><th>결과</th><th>실행</th></tr></thead><tbody>{runs.map(x => <tr key={x.id}><td style={{ whiteSpace: 'nowrap' }}>{fmtKST(x.started_at)}</td><td style={{ whiteSpace: 'nowrap' }}>{x.range_from?.slice(5)}~{x.range_to?.slice(5)}</td><td style={{ color: x.ok === false ? 'var(--bad)' : x.ok ? 'var(--ok)' : 'var(--muted)' }}>{x.message ?? '진행 중'}</td><td>{x.triggered_by}</td></tr>)}{runs.length === 0 && <tr><td colSpan={4} style={{ color: 'var(--muted)' }}>실행 이력 없음</td></tr>}</tbody></table></div>
