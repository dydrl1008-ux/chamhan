...
export async function saveTiers(fd: FormData): Promise<R> {
  await requireRole(['admin']);
  const scope = String(fd.get('scope')); if (!['staff', 'manager'].includes(scope)) return { ok: false, msg: 'scope 오류' };
  ...
  done(['/admin/criteria', '/criteria', '/me', '/admin/promotion']); return { ok: true, msg: `${scope === 'staff' ? '팀원' : '팀장'} 구간 ${rows.length}개 저장` };
}
export async function saveSettings(fd: FormData): Promise<R> {
  await requireRole(['admin']); const sb = supabaseServer();
  const rows = [['new_margin_bonus_rate', String(n(fd.get('new_margin_bonus_rate')) / 100)], ['promotion_doc', String(fd.get('promotion_doc') || '')], ['incentive_doc', String(fd.get('incentive_doc') || '')], ['late_after', String(fd.get('late_after') || '09:30')]];
