...
export default function PnlAdmin({ month, range, cats, mo, items, summary, marginSum, commission }: { month: string; range: [string, string]; commission: number; cats: { id: number; name: string }[]; mo: { operating_profit: number; note: string | null } | null; items: { category_id: number; amount: number; note: string | null }[]; summary: S[]; marginSum: number }) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 18 }}>
        <form className="card" action={async fd => { const x = await savePnl(fd); notify(x.msg); if (x.ok) r.refresh(); }}>
          ...
          <table><thead><tr><th>항목</th><th style={{ textAlign: 'right' }}>금액</th><th style={{ minWidth: 200 }}>비고</th></tr></thead><tbody>
            ...
            {cats.map(c => <tr key={c.id}><td>{c.name}<input type="hidden" name="category_id" value={c.id} />{c.name === '급여' && commission > 0 && <div style={{ fontSize: 11, color: 'var(--muted)' }}>참고: 정산 확정수수료 합 {won(commission)} <button type="button" className="btn ghost" style={{ padding: '1px 6px', fontSize: 11 }} onClick={() => setAmts({ ...amts, [c.id]: (amts[c.id] ?? 0) + commission })}>더하기</button></div>}</td><td><input name="amount" type="number" value={amts[c.id] ?? 0} onChange={e => setAmts({ ...amts, [c.id]: Number(e.target.value) || 0 })} style={{ textAlign: 'right', width: 160 }} /></td><td><input name="item_note" defaultValue={items.find(i => i.category_id === c.id)?.note ?? ''} placeholder="비고" style={{ padding: '6px 10px', fontSize: 12.5 }} /></td></tr>)}
