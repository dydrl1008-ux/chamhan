...
export default function PnlAdmin({ month, cats, mo, items, summary, marginSum }: { month: string; cats: { id: number; name: string }[]; mo: { operating_profit: number; note: string | null } | null; items: { category_id: number; amount: number; note: string | null }[]; summary: S[]; marginSum: number }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><h1 style={{ fontSize: 20, margin: 0 }}>영업이익 월별</h1><input type="month" value={month} onChange={e => r.push(`/admin/pnl?m=${e.target.value}`)} style={{ width: 160 }} /><span style={{ fontSize: 12, color: 'var(--muted)' }}>VAT 제외 · 실수익 = 영업이익 − 비용합계</span></div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 18 }}>
        <form className="card" action={async fd => { const x = await savePnl(fd); notify(x.msg); if (x.ok) r.refresh(); }}>
          ...
          <table><thead><tr><th>항목</th><th style={{ textAlign: 'right' }}>금액</th><th style={{ minWidth: 200 }}>비고</th></tr></thead><tbody>
            <tr><td><b>영업이익</b></td><td><input name="operating_profit" type="number" value={op} onChange={e => setOp(Number(e.target.value) || 0)} style={{ textAlign: 'right', width: 160 }} /></td><td><input name="note" defaultValue={mo?.note ?? ''} placeholder="비고" style={{ padding: '6px 10px', fontSize: 12.5 }} /><div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>참고: 이달 KPI 마진 합계 {won(marginSum)} <button type="button" className="btn ghost" style={{ padding: '1px 6px', fontSize: 11, marginLeft: 6 }} onClick={() => setOp(marginSum)}>가져오기</button></div></td></tr>
