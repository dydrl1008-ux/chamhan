      ...
      <div className="card">
        <table><thead><tr><th>라벨</th><th>IP / CIDR</th><th>등록</th><th>활성</th></tr></thead>
          <tbody>{rows.map(r => <tr key={r.id}><td><b>{r.label}</b></td><td style={{ fontFamily: 'monospace' }}>{r.cidr}</td><td style={{ fontSize: 12, color: 'var(--muted)' }}>{r.created_at.slice(0, 10)}</td>
            <td><input type="checkbox" checked={r.is_active} style={{ width: 'auto' }} onChange={async e => { const on = e.target.checked; if (!on && rows.filter(x => x.is_active).length === 1 && !confirm('마지막 활성 IP입니다. 끄면 모든 접속이 차단됩니다. 계속할까요?')) return; const res = await toggleIp(r.id, on); { notify(res.msg); setMsg(res.msg); }; }} /></td></tr>)}</tbody></table>
        <form action={async fd => { const r = await addIp(fd); { notify(r.msg); setMsg(r.msg); }; }} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 8, marginTop: 14 }}>
