...
export function notify(msg: string, kind?: 'ok' | 'bad', ms?: number) {
  if (typeof window === 'undefined' || !msg) return;
  const k = kind ?? (/실패|불가|에러|없|권한|부족|않|초과|이전|미래/.test(msg) ? 'bad' : 'ok');
  window.dispatchEvent(new CustomEvent('wh-toast', { detail: { msg, kind: k, ms: ms ?? (msg.length > 60 ? 12000 : 3200) } }));
}
export default function Toast() {
  const [items, setItems] = useState<{ id: number; msg: string; kind: string }[]>([]);
  useEffect(() => {
    const h = (e: Event) => { const { msg, kind, ms } = (e as CustomEvent).detail; const id = Date.now() + Math.random(); setItems(x => [...x, { id, msg, kind }]); setTimeout(() => setItems(x => x.filter(i => i.id !== id)), ms ?? 3200); };
